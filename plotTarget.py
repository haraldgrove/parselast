...
def plot_coverage(filename):
    ...
    df = pd.read_table(filename, comment='#', header=0, delim_whitespace=True, error_bad_lines=False)
    ...
    for (index,row) in df.iterrows():
        ...
        if row['qual'] == 1:
            axes.add_patch(patches.Rectangle((start,y),end-start,h,facecolor=color))
        else:
            axes.add_patch(patches.Rectangle((start,y),end-start,h,facecolor=color,hatch='*'))
        axes.text(start*1.0001,y+0.1,row['name2'],bbox={'facecolor':'white', 'alpha':1, 'pad':1})
        y += h
    scale_s = '{:.0f}'.format(0.1*(maxx-minx))
    scale = roundup(int(scale_s), len(scale_s)-1)
    axes.add_patch(patches.Rectangle((maxx-2*scale,0+0.25),scale,0.5,facecolor='black'))
    axes.text(maxx-1.9*scale , 0+0.3, '{} bp'.format(scale),color='white', size=18)

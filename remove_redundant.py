...
HEADER = 'score\tname1\tstart1\talnSize1\tstrand1\tseqSize1\tname2\tstart2\talnSize2\tstrand2\tseqSize2\tblocks\tEG\tE'
...
def split_lines(target, query):
    ...
    with open(infile, 'r') as fin, open(out1, 'w') as fout1, open(out2, 'w') as fout2:
        ...
        for line in fin:
            ...
            try:
                name1, start1, alnSize1, seqSize1 = line_l[1], int(line_l[2]), int(line_l[3]), int(line_l[5])
                name2, start2, alnSize2, seqSize2 = line_l[6], int(line_l[7]), int(line_l[8]), int(line_l[10])
            except ValueError:
                continue
            if alnSize1 >= seqSize1 and alnSize2 < seqSize2 and seqSize1 < seqSize2 * 0.9:
                fout2.write('{}\t{}\n'.format(name2, name1))
            if alnSize2 >= seqSize2 and alnSize1 < seqSize1 and seqSize2 < seqSize1 * 0.9:
                fout1.write('{}\t{}\n'.format(name1, name2))
